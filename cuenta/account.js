(() => {
  ...
  function setupRecords(me,openReport){
    const kinds={topup:'Recarga con QR',yape_topup:'Recarga Yape',mp_topup:'Recarga Mercado Pago',purchase:'Compra',refund:'Devolución'};
    for(const key of ['recargas','historial','reportes']){
      const isReport=key==='reportes',records=isReport?(me.reports||[]):me.movements.filter(m=>key==='historial'||['topup','yape_topup','mp_topup'].includes(m.kind));
      ...
      const matches=(m,status)=>status==='all'||(isReport?m.status===status:key==='historial'?(status==='topups'?['topup','yape_topup','mp_topup'].includes(m.kind):m.kind===status):status==='waiting'?['pending','yape_pending','mp_pending'].includes(m.status):status==='review'?['review','attention','expired'].includes(m.status):m.status===status);
