(() => {
  ...
  async function dashboard(){
    ...
    let amount=product?Math.round(Number(product.pen)*100):0;const originalAmount=amount;let appliedCoupon='';
    ...
    const tutorialsSection=document.createElement('section');tutorialsSection.dataset.customerSection='tutoriales';tutorialsSection.className='settings-card';tutorialsSection.innerHTML='<h2>Tutoriales</h2><p>Aprende a usar tu cuenta y los servicios de la tienda.</p><div id="customerTutorials" role="status">Cargando tutoriales…</div>';root.querySelector('.portal-workspace').append(tutorialsSection);
    fetch('/api/tutorials',{cache:'no-store'}).then(async r=>{if(!r.ok)throw Error('No se pudieron cargar los tutoriales.');return r.json();}).then(data=>{tutorialsSection.querySelector('#customerTutorials').innerHTML=data.items.map(t=>`<article style="margin-top:24px"><h3>${esc(t.title)}</h3><video controls playsinline preload="metadata" style="width:100%;max-width:800px;border-radius:14px;margin-top:12px;background:#090d14" src="/api/tutorials/${esc(t.id)}"></video></article>`).join('')||'<p>Todavía no hay tutoriales publicados.</p>';}).catch(e=>{tutorialsSection.querySelector('#customerTutorials').textContent=e.message;});
    setupPortal();
    $('#balanceCouponForm').onsubmit=async e=>{e.preventDefault();if(busy)return;locked(true);try{const result=await api('coupons/redeem',{code:new FormData(e.target).get('code')});await dashboard();toast('Recibiste '+money(result.amount_cents)+' de saldo.');}catch(error){$('#balanceCouponStatus').textContent=error.message;}finally{locked(false);}};
    function resetDiscount(){amount=originalAmount;$('#insufficientBalance').hidden=user.balance_cents>=amount;appliedCoupon='';checkoutRequest=null;$('#checkoutPrice').textContent=money(amount);$('#checkoutTotal').textContent=money(amount);const b=$('#confirmPurchase');b.textContent='🔒 Confirmar compra · '+money(amount);b.disabled=user.balance_cents<amount;if(b.disabled)b.dataset.unavailable='true';else delete b.dataset.unavailable;}
    if($('#discountCode'))$('#discountCode').oninput=()=>{resetDiscount();$('#discountStatus').textContent='';};
    if($('#applyDiscount'))$('#applyDiscount').onclick=async()=>{if(busy)return;resetDiscount();locked(true);$('#discountCode').disabled=true;try{const q=await api('coupons/quote',{code:$('#discountCode').value,product_id:product.id});amount=q.amount_cents;$('#insufficientBalance').hidden=user.balance_cents>=amount;appliedCoupon=q.code;$('#checkoutPrice').textContent=money(amount);$('#checkoutTotal').textContent=money(amount);$('#discountStatus').textContent='Descuento: '+money(q.discount_cents);const b=$('#confirmPurchase');b.textContent='🔒 Confirmar compra · '+money(amount);if(user.balance_cents>=amount)delete b.dataset.unavailable;else b.dataset.unavailable='true';}catch(error){$('#discountStatus').textContent=error.message;}finally{$('#discountCode').disabled=false;locked(false);}};
    ...
    if($('#confirmPurchase'))$('#confirmPurchase').onclick=async()=>{
      ...
      try{if(!checkoutRequest){try{checkoutRequest=JSON.parse(sessionStorage.getItem(key));}catch{}if(!checkoutRequest||checkoutRequest.expected_cents!==amount||(checkoutRequest.coupon_code||'')!==appliedCoupon)checkoutRequest={request_id:crypto.randomUUID(),product_id:product.id,expected_cents:amount,coupon_code:appliedCoupon};try{sessionStorage.setItem(key,JSON.stringify(checkoutRequest));}catch{}}
  ...
  const portalGroups=[['tutorials','Tutoriales','eye',[['tutoriales','Ver tutoriales']]],['account','Mi cuenta','user',[['acceso','Mi perfil']]],['orders','Gestionar pedidos','bag',[['compras','Mis cuentas y compras']]],['wallet','Gestionar recargas','wallet',[['billetera','Recargar'],['recargas','Mis recargas']]],['history','Historial','history',[['historial','Mi historial']]]];
  ...
  function setupPortal(){
    const titles={tutoriales:'Tutoriales',acceso:'Mi cuenta',compras:'Mis cuentas y compras',billetera:'Agregar saldo',recargas:'Mis recargas',historial:'Mi historial',reportes:'Mis reportes'};
    ...
    navigateSection=section=>{
      ...
      root.querySelectorAll('[data-customer-section]').forEach(el=>{el.hidden=el.dataset.customerSection!==section;if(el.hidden)el.querySelectorAll('video').forEach(video=>video.pause());});
  ...
  function setupRecords(me,openReport){
    const kinds={coupon_topup:'Cupón de saldo',topup:'Recarga con QR',yape_topup:'Recarga Yape',mp_topup:'Recarga Mercado Pago',admin_topup:'Saldo agregado por la tienda',purchase:'Compra',renewal:'Renovación de cuenta',refund:'Devolución'};
    for(const key of ['recargas','historial','reportes']){
      const isReport=key==='reportes',records=isReport?(me.reports||[]):me.movements.filter(m=>key==='historial'||['topup','yape_topup','mp_topup','admin_topup','coupon_topup'].includes(m.kind));
  ...
  function setupOrders(orders,reports){
    ...
    const createdLabel=value=>{
      ...
      return 'Se activó '+(sameDay?'hoy':'el '+created.toLocaleDateString('es-PE',{timeZone:'America/Lima'}))+' a las '+time;
    };
    ...
    function openDetails(o){
      ...
      box.innerHTML=`<div class="order-product"><div class="order-product-logo"><img src="${esc(asset(o.product_logo||'logo/arcangel-us.png'))}" alt="Logo de ${esc(o.product_name)}"></div><div><h3>${esc(o.product_name)}</h3><p>${esc(o.product_brand||o.product_type||'Cuenta de la tienda')}</p></div></div>
        ...
        <section class="order-detail-section"><h3>Detalles del Pedido</h3><dl class="order-facts-grid">
          ...
          ${fact('Fecha de activación',createdLabel(o.created_at),'calendar')}
