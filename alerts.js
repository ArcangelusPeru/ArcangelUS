(() => {
  ...
  function setSound(value){sound=value;el('enableSalesSound').textContent=value?'Sonido activado · silenciar':'Activar sonido';el('enableSalesSound').setAttribute('aria-pressed',String(value));}
  ...
  async function refresh(){
    ...
    try{
      ...
      const incoming=new Set([...data.requests.map(r=>'topup:'+r.entry_id),...(data.yape||[]).map(r=>'yape:'+r.claim_id),...(data.reports||[]).map(r=>'report:'+r.report_id)]),fresh=seen!==null&&[...incoming].some(id=>!seen.has(id));seen=incoming;
  ...
  function stop(){generation++;ctx=null;seen=null;polling=false;clearInterval(timer);timer=null;setSound(false);el('salesAlert').hidden=true;if(audio){audio.close().catch(()=>{});audio=null;}}
  function start(context){
    ctx=context;if(timer)return;generation++;
    el('enableSalesSound').onclick=async()=>{
      if(sound){setSound(false);return;}
      try{const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)throw Error();if(!audio)audio=new Audio();await audio.resume();setSound(true);await chime();}catch{el('salesAlertStatus').textContent='El navegador no pudo activar el sonido. Las solicitudes seguirán apareciendo aquí.';setSound(false);}
    };
  ...
  window.addEventListener('focus',refresh);window.addEventListener('pagehide',()=>{if(audio)audio.suspend().catch(()=>{});});
  window.arcangelAlerts={start,stop,refresh};
})();
