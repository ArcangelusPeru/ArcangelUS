(() => {
  ...
  function openProduct(id,duplicate=false){
    let p=id?copy(state.products.find(p=>String(p.id)===String(id))):{id:crypto.randomUUID(),name:'',brand:'',sub:'',description:'',type:'',duration:'1 mes',filter:state.categories[0]?.slug||'',pen:0,original_pen:null,stock_quantity:null,banner_url:'',features:[],note:'',active:true,out_of_stock:false,is_oferta:false,checkout_mode:'off',sort_order:Math.max(0,...state.products.map(p=>p.sort_order))+10,cat_sort_order:0,palette:''};
