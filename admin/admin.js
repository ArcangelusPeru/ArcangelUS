(() => {
  ...
  function render(){
    if(!state)return;
    // Hide only the controls; both announcement services keep running across views.
    $('#notificationControls').hidden=view!=='yapeActivity';
    ...
    const labels={fazer:['FazerCards','Conecta el proveedor y revisa sus productos y costos.'],tutorials:['Tutoriales','Publica videos con títulos para tus clientes.'],coupons:['Cupones','Crea descuentos y regalos de saldo con límite de usos y vencimiento.'],products:['Productos','Administra tu catálogo, sus precios y su disponibilidad.'],categories:['Categorías','Organiza tus productos y cambia las imágenes de los filtros.'],settings:['Configurar web','Edita la marca, los textos, el contacto y el fondo de tu tienda.'],backups:['Respaldos','Descarga una copia de tus productos, configuración e imágenes subidas.'],customers:['Clientes','Gestiona usuarios, roles, contraseñas y saldo.'],payments:['Pagos','Gestiona las recargas de saldo y Yape automático.'],commerce:['Ventas','Gestiona cuentas disponibles, ventas, reportes y reemplazos.'],yapeActivity:['Notificaciones Yape','Todos tus avisos de pago, organizados en un solo lugar.']};
    ...
    if(view==='fazer')window.renderFazer({api,esc,current:()=>view==='fazer'});else if(view==='tutorials')window.renderTutorials({api,esc,toast,current:()=>view==='tutorials'});else if(view==='coupons')window.renderCoupons({api,esc,toast,state:()=>state,current:()=>view==='coupons',dirty:value=>dirty=value});else if(view==='products')renderProducts();else if(view==='categories')renderCategories();else if(view==='backups')renderBackups();else if(view==='commerce'||view==='customers'||view==='payments')renderCommerce();else if(view==='yapeActivity')renderYapeActivity();else renderSettings();
  ...
  function reorderMode(){return category?'category':'store';}
  function canReorder(){return !query.trim()&&!status;}
  function renderRows(){
    ...
    list.sort((a,b)=>mode==='category'?((a.cat_sort_order??0)-(b.cat_sort_order??0))||((a.sort_order??0)-(b.sort_order??0)):((a.sort_order??0)-(b.sort_order??0)));
    const draggable=canReorder();
    $('#productRows').innerHTML=list.length?list.map(p=>`<tr data-product-row="${esc(p.id)}" class="${draggable?'is-draggable':''}"><td><div class="product-ident"><button class="drag-handle" type="button" draggable="${draggable?'true':'false'}" aria-label="Arrastrar ${esc(p.name)}" title="Arrastra para ordenar" ${draggable?'':'aria-disabled="true"'}>⠿</button><img src="${esc(asset(p.banner_url))}" alt="" loading="lazy"><div><strong>${esc(p.name)}</strong><small>${esc([p.type,p.duration].filter(Boolean).join(' · '))}</small></div></div></td><td class="category-text">${esc(catName(p.filter))}</td><td class="money"><small>Cliente</small>${money(p.pen)}<small>Revendedor: ${money(p.reseller_pen??p.pen)}</small></td><td><span class="badge ${availability(p)}">${statusLabel[availability(p)]}</span>${p.stock_quantity!=null?`<small class="stock-units">${p.stock_quantity} unidades</small>`:''}</td><td><div class="row-actions"><button class="button small secondary" data-edit="${esc(p.id)}" aria-label="Editar ${esc(p.name)}">Editar</button><button class="button small ghost duplicate" data-duplicate="${esc(p.id)}" aria-label="Duplicar ${esc(p.name)}">Duplicar</button></div></td></tr>`).join(''):'<tr><td colspan="5"><div class="empty"><strong>No hay productos</strong>Cambia los filtros o agrega tu primer producto.</div></td></tr>';
    $('#reorderHint').textContent=draggable?(category?'Arrastra el asa ⠿ para ordenar los productos dentro de esta categoría.':'Arrastra el asa ⠿ para ordenar los productos de la tienda. El cambio se guarda automáticamente.'):'Quita la búsqueda y el filtro de disponibilidad para poder arrastrar y ordenar.';
    ...
    if(!draggable)return;
    let draggedId='';
    $$('.drag-handle').forEach(handle=>handle.addEventListener('dragstart',event=>{
      draggedId=handle.closest('[data-product-row]').dataset.productRow;event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',draggedId);handle.closest('tr').classList.add('dragging');
    }));
    $$('[data-product-row]').forEach(row=>{
      row.addEventListener('dragover',event=>{if(!draggedId||row.dataset.productRow===draggedId)return;event.preventDefault();event.dataTransfer.dropEffect='move';$$('[data-product-row]').forEach(item=>item.classList.remove('drag-over'));row.classList.add('drag-over');});
      row.addEventListener('drop',async event=>{event.preventDefault();const targetId=row.dataset.productRow;if(!draggedId||targetId===draggedId)return;const before=event.clientY<row.getBoundingClientRect().top+row.getBoundingClientRect().height/2;const sourceId=draggedId;draggedId='';await moveProduct(sourceId,targetId,before,list,mode);});
      row.addEventListener('dragend',()=>{draggedId='';$$('[data-product-row]').forEach(item=>item.classList.remove('dragging','drag-over'));});
    });
  }
  async function moveProduct(sourceId,targetId,before,list,mode){
    const ids=list.map(p=>String(p.id));const from=ids.indexOf(String(sourceId)),to=ids.indexOf(String(targetId));if(from<0||to<0)return;
    ids.splice(from,1);let destination=ids.indexOf(String(targetId));if(!before)destination++;ids.splice(destination,0,String(sourceId));
    const next=copy(state),byId=new Map(next.products.map(p=>[String(p.id),p]));
    if(mode==='category')ids.forEach((id,index)=>{byId.get(id).cat_sort_order=(index+1)*10;});
    else {const ordered=ids.map(id=>byId.get(id));const visible=new Set(ids);let cursor=0;next.products=next.products.map(product=>visible.has(String(product.id))?ordered[cursor++]:product);next.products.forEach((product,index)=>{product.sort_order=(index+1)*10;});}
    try{if(await save(next))renderRows();}catch(error){toast(error.message,true);renderRows();}
  }
  ...
  function openProduct(id,duplicate=false){
    let p=id?copy(state.products.find(p=>String(p.id)===String(id))):{id:crypto.randomUUID(),name:'',brand:'',sub:'',description:'',type:'',duration:'1 mes',filter:state.categories[0]?.slug||'',pen:0,original_pen:null,stock_quantity:null,banner_url:'',features:[],note:'',active:true,out_of_stock:false,is_oferta:false,checkout_mode:'off',whatsapp_enabled:true,sort_order:Math.max(0,...state.products.map(p=>p.sort_order))+10,cat_sort_order:0,palette:''};
    ...
    $('#editorFields').innerHTML=`<div class="editor-layout"><div><section class="form-section"><h3>Información del producto</h3>${field('name','Nombre',p.name,{required:true,max:160})}<div class="field-grid">${field('brand','Marca o plataforma',p.brand,{max:100})}<label class="field"><span>Categoría *</span><select name="filter" required>${state.categories.map(c=>`<option value="${esc(c.slug)}" ${c.slug===p.filter?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label>${field('type','Tipo de cuenta o licencia',p.type,{max:150})}${field('duration','Duración',p.duration,{max:100})}</div><div style="margin-top:16px">${field('sub','Subtítulo',p.sub,{max:250})}${field('description','Descripción completa',p.description,{area:true,max:12000,help:'Se muestra en la ficha del producto.'})}</div></section><section class="form-section"><h3>Precio y stock</h3><div class="field-grid">${field('pen','Precio CLIENTE (S/)',p.pen,{type:'number',required:true})}${field('reseller_pen','Precio REVENDEDOR (S/)',p.reseller_pen??p.pen,{type:'number',required:true,help:'Solo lo verá el usuario con rol Revendedor.'})}${field('original_pen','Precio anterior CLIENTE',p.original_pen??'',{type:'number',help:'Opcional. Se muestra tachado al cliente.'})}${field('reseller_original_pen','Precio anterior REVENDEDOR',p.reseller_original_pen??'',{type:'number',help:'Opcional. Se muestra tachado al revendedor.'})}<label class="field"><span>Disponibilidad</span><select name="availability" id="availability"><option value="available" ${!p.out_of_stock?'selected':''}>Disponible</option><option value="soldout" ${p.out_of_stock?'selected':''}>Agotado</option></select></label>${field('stock_quantity','Unidades disponibles',p.stock_quantity??'',{type:'number',step:'1',help:'Opcional. Con 0 se marca agotado. Actualiza las unidades después de cada venta por WhatsApp.'})}</div><label class="check-field"><input name="active" type="checkbox" ${p.active?'checked':''}>Mostrar producto en la tienda</label><label class="check-field"><input name="is_oferta" type="checkbox" ${p.is_oferta?'checked':''}>Incluir en Promos y Ofertas</label></section><section class="form-section"><h3>Canales de venta</h3><label class="check-field"><input name="checkout_mode" type="checkbox" ${p.checkout_mode==='automatic'?'checked':''}>Mostrar botón COMPRAR ACÁ (saldo y entrega automática)</label><label class="check-field"><input name="whatsapp_enabled" type="checkbox" ${p.whatsapp_enabled!==false?"checked":""}>Mostrar botón COMPRAR POR WHATSAPP</label><p class="help">Marca los botones que quieres mostrar, tanto en el catálogo como en Ver más. COMPRAR ACÁ entrega una cuenta disponible del inventario.</p></section><section class="form-section"><h3>Detalles y orden</h3>${field('features','Características incluidas',(p.features||[]).join('\n'),{area:true,rows:5,help:'Una característica por línea.'})}${field('note','Nota o condiciones',p.note,{area:true,max:12000})}<div class="field-grid">${field('sort_order','Posición en la tienda',p.sort_order,{type:'number',step:'1',help:'También puedes arrastrar los productos desde la lista para ordenarlos.'})}${field('cat_sort_order','Posición en su categoría',p.cat_sort_order,{type:'number',step:'1',help:'También puedes arrastrar desde una categoría filtrada.'})}</div></section></div><aside class="editor-sidebar"><div><h3 style="font-size:15px;margin-bottom:12px">Imagen del producto</h3>${imageEditor('banner_url',p.banner_url,'Vista previa del producto')}</div><div><h3 style="font-size:15px;margin-bottom:12px">Logo de la plataforma</h3><p class="help">Se muestra junto a cada cuenta del inventario. Si lo dejas vacío, se usa la imagen del producto.</p>${imageEditor('logo_url',p.logo_url,'Logo de la plataforma')}</div><div class="mini-preview"><p style="margin-bottom:12px">VISTA PREVIA</p><strong id="previewName">${esc(p.name||'Nombre del producto')}</strong><p id="previewDetail">${esc([p.type,p.duration].filter(Boolean).join(' · '))}</p><div class="preview-price" id="previewPrice">${money(p.pen)}</div></div></aside></div>`;
  ...
  function renderBackups(){
    $('#mainContent').innerHTML=`<section class="settings-card"><h2>Guardar una copia del catálogo</h2><p class="section-note">Catálogo: <strong>${esc(storage.catalogId)}</strong>. ${storage.kind==='mysql'?'Los cambios y las imágenes subidas se guardan en MySQL.':'Los cambios se guardan en este equipo.'} Este respaldo incluye solo catálogo e imágenes subidas. Para clientes, saldos, cuentas y ventas usa Exportar SQL en GoDaddy y conserva COMMERCE_KEY por separado.</p><a class="button primary" href="/api/admin/backup" download>Descargar respaldo del catálogo</a></section><section class="settings-card"><h2>Restaurar una copia</h2>${importField()}</section><section class="settings-card"><h2>Versiones anteriores</h2><p class="section-note">Se conservan las últimas 50 versiones del catálogo en MySQL. Descarga una copia antes de restaurarla.</p><div id="backupList">Consultando versiones…</div></section>`;
    wireImport();
  ...
  $('#editorForm').addEventListener('submit',async e=>{
    ...
    if(editing.type==='product'){
      const p={...editing.item};for(const key of ['name','brand','sub','description','type','duration','filter','banner_url','logo_url','note'])p[key]=String(f.get(key)||'').trim();
      for(const key of ['pen','reseller_pen','sort_order','cat_sort_order'])p[key]=Number(f.get(key));p.original_pen=f.get('original_pen')===''?null:Number(f.get('original_pen'));p.reseller_original_pen=f.get('reseller_original_pen')===''?null:Number(f.get('reseller_original_pen'));p.stock_quantity=f.get('stock_quantity')===''?null:Number(f.get('stock_quantity'));p.features=String(f.get('features')||'').split('\n').map(s=>s.trim()).filter(Boolean);p.active=f.has('active');p.is_oferta=f.has('is_oferta');p.whatsapp_enabled=f.has('whatsapp_enabled');p.checkout_mode=f.has('checkout_mode')?'automatic':'off';p.out_of_stock=f.get('availability')==='soldout'||p.stock_quantity===0;
  ...
  boot().catch(e=>{showLogin();$('#loginError').textContent=e.message;$('#mainContent').innerHTML='';});
})();
