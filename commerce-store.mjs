...
import { fail, digest, delivery, email, hashPassword, verifyPassword, secretToken, equalSecret, cents, soles, text, requestId } from './commerce-security.mjs';
...
export async function createCommerceStore({pool,transaction,catalogId:cat,sealer,updateCatalog}){
  ...
  for(const ddl of [
    ...
    `arcangel_users (${catalog},user_id ${uuid} NOT NULL,email VARCHAR(254) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,password_hash VARCHAR(220) NOT NULL,recovery_hash CHAR(64) NOT NULL,balance_cents BIGINT UNSIGNED NOT NULL DEFAULT 0,blocked BOOLEAN NOT NULL DEFAULT FALSE,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(catalog_id,user_id),UNIQUE KEY email_unique(catalog_id,email))`,
    ...
    `arcangel_ledger (${catalog},entry_id ${uuid} NOT NULL,user_id ${uuid} NOT NULL,kind VARCHAR(32) NOT NULL,amount_cents BIGINT NOT NULL,status VARCHAR(24) NOT NULL,reference VARCHAR(120) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,note VARCHAR(1000) NOT NULL DEFAULT '',created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(catalog_id,entry_id),UNIQUE KEY reference_unique(catalog_id,reference),KEY ledger_user(catalog_id,user_id,created_at))`,
    `arcangel_inventory (${catalog},inventory_id ${uuid} NOT NULL,product_id VARCHAR(100) NOT NULL,secret TEXT NOT NULL,fingerprint CHAR(64) NOT NULL,state VARCHAR(24) NOT NULL DEFAULT 'available',order_id ${uuid} NULL,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(catalog_id,inventory_id),UNIQUE KEY inventory_unique(catalog_id,fingerprint),KEY inventory_product(catalog_id,product_id,state))`,
  ...
  const profile=user=>({id:user.user_id,email:user.email,balance_cents:Number(user.balance_cents),blocked:!!user.blocked});
  ...
  const orderView=row=>({order_id:row.order_id,product_id:row.product_id,product_name:row.product_name,amount_cents:Number(row.amount_cents),delivery_mode:row.delivery_mode,status:row.status,created_at:row.created_at});
  return {
    ...
    async register(address,password){
      address=email(address);const passwordHash=await hashPassword(password),recovery=secretToken(),id=randomUUID();
      try{return await transaction(async db=>{
        await query(db,'INSERT INTO arcangel_users(catalog_id,user_id,email,password_hash,recovery_hash,recovery_secret) VALUES(?,?,?,?,?,?)',[id,address,passwordHash,digest(recovery),sealer.seal(recovery,`${cat}:recovery:${id}`)]);
        return {...await session(db,{user_id:id,email:address,balance_cents:0,blocked:false}),recovery_code:recovery};
      });}catch(error){duplicate(error);}
    },
    async createCustomer(address,password){
      address=email(address);const passwordHash=await hashPassword(password),recovery=secretToken(),id=randomUUID();
      try{return await transaction(async db=>{
        await catalogLock(db);
        await query(db,'INSERT INTO arcangel_users(catalog_id,user_id,email,password_hash,recovery_hash,recovery_secret) VALUES(?,?,?,?,?,?)',[id,address,passwordHash,digest(recovery),sealer.seal(recovery,`${cat}:recovery:${id}`)]);
        return {customer:profile({user_id:id,email:address,balance_cents:0,blocked:false}),recovery_code:recovery};
      });}catch(error){duplicate(error);}
    },
    async login(address,password){
      address=email(address);const [user]=await query(pool,'SELECT * FROM arcangel_users WHERE catalog_id=? AND email=?',[address]);
      const valid=await verifyPassword(password,user?.password_hash);if(!valid||!user||user.blocked||user.deleted_at)throw fail(401,'Correo o contraseña incorrectos, o acceso desactivado.');
    ...
    async userFromToken(token){
      ...
      const [user]=await query(pool,'SELECT u.* FROM arcangel_users u JOIN arcangel_user_sessions s ON s.catalog_id=u.catalog_id AND s.user_id=u.user_id WHERE u.catalog_id=? AND s.token_hash=? AND s.expires_at>NOW() AND u.blocked=FALSE AND u.deleted_at IS NULL',[digest(token)]);
    ...
    async inventory(productId=''){
      const rows=await query(pool,'SELECT inventory_id,product_id,state,order_id,created_at,secret FROM arcangel_inventory WHERE catalog_id=?'+(productId?' AND product_id=?':'')+' ORDER BY created_at DESC,inventory_id LIMIT 500',productId?[productId]:[]);
      // Only the owner route calls this list. Keep passwords on the explicit reveal route.
      return rows.map(({secret,...row})=>{const d=delivery(sealer.open(secret,`${cat}:inventory:${row.inventory_id}`));return {...row,username:d.username,url:d.url||'',profile:d.profile||'',pin:d.pin||'',has_password:!!d.password};});
    },
    ...
    async inventoryDetails(id){
      id=requestId(id);const [row]=await query(pool,'SELECT secret,state FROM arcangel_inventory WHERE catalog_id=? AND inventory_id=?',[id]);if(!row)throw fail(404,'Cuenta no encontrada.');
      return {delivery:delivery(sealer.open(row.secret,`${cat}:inventory:${id}`)),revision:sealer.mac(row.state+':'+row.secret),state:row.state};
    },
    async updateInventory(id,value,revision){
      ...
      return transaction(async db=>{
        ...
        if(!['available','sold','external'].includes(row.state))throw fail(409,'Esta cuenta está retirada.');
    ...
    async purchase(userId,input){
      ...
      return transaction(async db=>{
        ...
        if(old){if(old.user_id!==userId||old.product_id!==productId||Number(old.amount_cents)!==expected)throw fail(409,'La operación ya existe con otros datos.');return {order:orderView(old),balance_cents:Number(user.balance_cents)};}
        const product=snapshot.data.products.find(p=>p.id===productId);
        if(!product||!product.active||!['manual','automatic'].includes(product.checkout_mode))throw fail(409,'Este producto no admite compras con saldo.');
        const amount=soles(product.pen);if(amount!==expected)throw fail(409,'El precio cambió. Revisa el precio y confirma nuevamente.');
        ...
        let inventory=null,secret=null;
        if(product.checkout_mode==='automatic'){
          [inventory]=await query(db,"SELECT * FROM arcangel_inventory WHERE catalog_id=? AND product_id=? AND state='available' ORDER BY created_at,inventory_id LIMIT 1 FOR UPDATE",[productId]);
          if(!inventory)throw fail(409,'No quedan cuentas disponibles.');
          secret=sealer.seal(sealer.open(inventory.secret,`${cat}:inventory:${inventory.inventory_id}`),`${cat}:order:${id}`);
        }
        const reserved=product.checkout_mode==='manual'&&Number.isInteger(product.stock_quantity);
        if(reserved){product.stock_quantity--;product.out_of_stock=product.stock_quantity===0;}
        const status=inventory?'delivered':'pending_manual';
        ...
        return {order:{order_id:id,product_id:productId,product_name:product.name,amount_cents:amount,delivery_mode:product.checkout_mode,status},balance_cents:Number(user.balance_cents)-amount};
      });
    },
    async orders(userId){
      const rows=await query(pool,'SELECT * FROM arcangel_orders WHERE catalog_id=? AND user_id=? ORDER BY created_at DESC,order_id LIMIT 200',[userId]);
      ...
      return rows.map(row=>{
        ...
        if(row.status==='delivered'&&row.delivery_secret){
          ...
          account={username:d.username,url:d.url||'',profile:d.profile||'',pin:d.pin||'',notes:d.notes,has_password:!!d.password,starts_on:d.starts_on||'',expires_on:d.expires_on||''};
        }
        return {...orderView(row),product_logo:product?.logo_url||product?.banner_url||'',account};
      });
    },
    ...
    async reports(userId=null){
      const rows=await query(pool,"SELECT r.*,o.product_name,o.status AS order_status,o.delivery_secret,u.email FROM arcangel_reports r JOIN arcangel_orders o ON o.catalog_id=r.catalog_id AND o.order_id=r.order_id JOIN arcangel_users u ON u.catalog_id=r.catalog_id AND u.user_id=r.user_id WHERE r.catalog_id=?"+(userId?' AND r.user_id=?':'')+" ORDER BY (r.status<>'resolved') DESC,r.created_at DESC,r.report_id LIMIT 200",userId?[userId]:[]);
    ...
    async customers(search='',status='all'){
      ...
      const rows=await query(pool,"SELECT user_id,email,balance_cents,blocked,created_at,deleted_at FROM arcangel_users WHERE catalog_id=? AND "+filters[status]+" AND email LIKE ? ESCAPE '!' ORDER BY created_at DESC,user_id LIMIT 200",['%'+escaped+'%']);return rows.map(row=>({...profile(row),created_at:row.created_at,deleted_at:row.deleted_at}));
    },
    ...
    async adminData(){
      const topups=await query(pool,"SELECT l.*,u.email FROM arcangel_ledger l JOIN arcangel_users u ON u.catalog_id=l.catalog_id AND u.user_id=l.user_id WHERE l.catalog_id=? AND l.kind IN ('topup','mp_topup','yape_topup') ORDER BY (l.status='attention') DESC,(l.status='pending') DESC,l.created_at DESC,l.entry_id LIMIT 200");
      const orders=await query(pool,"SELECT o.*,u.email FROM arcangel_orders o JOIN arcangel_users u ON u.catalog_id=o.catalog_id AND u.user_id=o.user_id WHERE o.catalog_id=? ORDER BY (o.status='pending_manual') DESC,o.created_at DESC,o.order_id LIMIT 200");
      ...
      return {summary:Object.fromEntries(Object.entries(summary).map(([key,value])=>[key,Number(value)])),topups:topups.map(row=>({...row,amount_cents:Number(row.amount_cents)})),orders:orders.map(row=>({...orderView(row),email:row.email})),inventory:inventory.map(row=>({...row,units:Number(row.units)}))};
    },
  };
}
