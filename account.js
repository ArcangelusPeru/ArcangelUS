(() => {
  ...
  function setupOrders(orders,reports){
    ...
    function openDetails(o){
      ...
      dialog.classList.add('order-detail-dialog');$('#orderInfoTitle').textContent='Detalles de la compra: #'+reference;
      box.innerHTML=`<div class="order-product"><div class="order-product-logo"><img src="${esc(asset(o.product_logo||'logo/arcangel-us.png'))}" alt="Logo de ${esc(o.product_name)}"></div><div><h3>${esc(o.product_name)}</h3><p>${esc(o.product_brand||o.product_type||'Cuenta de la tienda')}</p></div></div>
        <section class="order-detail-section"><h3>Contacto y soporte</h3><div class="order-contact-box">${supportHref?`<a class="order-contact-button" href="${esc(supportHref)}" target="_blank" rel="noopener noreferrer">${icon('support')} CONTACTAR POR WHATSAPP</a>`:'<button type="button" class="order-contact-button" data-detail-support>CONTACTAR SOPORTE</button>'}</div></section>
        <section class="order-detail-section"><h3>Detalles de la compra</h3><dl class="order-facts-grid">
